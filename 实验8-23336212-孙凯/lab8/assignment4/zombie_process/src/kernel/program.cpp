...
int ProgramManager::wait(int *retval)
{
    ...
    while (true)
    {
        ...
        while (item)
        {
            ...
            if ((child->parentPid == this->running->pid)||(this->running->pid == 1 && child->status == ProgramStatus::DEAD))
            {
                flag = false;
